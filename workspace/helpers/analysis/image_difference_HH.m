...
if(exist('MaskNum','var')==0), MaskNum=numel(V); end  % CHANGED BY HH Feb 12, 2025
...
% if(~isempty(Mask)), I=I.*Mask;  end
% % t=sum(I(:))/MaskNum;
% t=sum(I(:));  % CHANGED BY HOOMHAM 08/25/2023

% CHANGED BY HOOMHAM Feb 12, 2025
V(Mask==0) = NaN; U(Mask==0) = NaN;
Vvar=V-tsnanmean(V(:)); Uvar=U-tsnanmean(U(:));

t=(tsnansum(Vvar(:).*Uvar(:)))/(sqrt(tsnansum(Vvar(:).^2) * tsnansum(Uvar(:).^2)) + eps);
I=[];
...
if(size(U,3)<4)
    ...
    for i=1:size(U,3)
        if(i==1)
            I(:,:,i)=(1/2)*(registration_error_normalized_cross_correlation(conv2(V(:,:,i),Gx,'same'),conv2(U(:,:,i),Gx,'same'),Mask,MaskNum)...
                +registration_error_normalized_cross_correlation(conv2(V(:,:,i),Gy,'same'),conv2(U(:,:,i),Gy,'same'),Mask,MaskNum));
        end
    end
else
    [Gx,Gy,Gz]=sobel3();
    I=(1/3)*(registration_error_normalized_cross_correlation(convn(V,Gx,'same'),convn(U,Gx,'same'), Mask,MaskNum)...
        +registration_error_normalized_cross_correlation(convn(V,Gy,'same'),convn(U,Gy,'same'),Mask,MaskNum)...
        +registration_error_normalized_cross_correlation(convn(V,Gz,'same'),convn(U,Gz,'same'),Mask,MaskNum));
end
if(~isempty(Mask)), I=I.*Mask;  end
t=sum(I(:))/MaskNum;
...
sizemask = (size(Mask) - size(I))/2;  % ADDED BY HOOMHAM 08/25/2023
Mask1 = (Mask( sizemask:size(Mask,1)-sizemask-1, sizemask:size(Mask,2)-sizemask-1, sizemask:size(Mask,3)-sizemask-1));  % ADDED BY HOOMHAM 08/25/2023
if(~isempty(Mask)), I=I.*Mask1;  end
t=sum(I(:))/MaskNum;
...
if(~isempty(Mask)),
    sizes=size(V);
    Vf=V(:); Vf(~Mask)=[]; Uf=U(:); Uf(~Mask)=[]; 
    if(isempty(Vf)), t=0; I=[]; return, end
