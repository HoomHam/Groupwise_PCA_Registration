...
# def orchestrate_registration_workflow(image, clahe, dp_rbc, dp_mem, savedir, target_weights, index_sets, cyclic):
...
#     print(savedir)
#     image4D, mask4D, registeror4D, clahe4D, Te = register_groupwise(image, enhanced_image4d, clahe, dp_rbc, dp_mem, os.path.join(save_dir, 'groupwise/0/'), time_threshold=1, cyclic = cyclic)
    ...
    # image4D, mask4D, registeror4D, clahe4D, Tn = register_groupwise(image4D, nreduced_image, clahe4D, dp_rbc, dp_mem, os.path.join(save_dir, 'groupwise/1/'), time_threshold=1, cyclic = cyclic)
        
    # print('CLAHE...')   
    # # Use CLAHE as the registeror  
    # sitk.WriteImage(clahe4D, os.path.join(savedir, "ns_clahe_init.nii")) 
    # image4D, mask4D, registeror4D, clahe4D, Tc = register_groupwise(image4D, clahe4D, clahe4D, dp_rbc, dp_mem, os.path.join(save_dir, 'groupwise/2/'), time_threshold=1, cyclic = cyclic)

    # print('Image...')           
    # # Use the image as the registeror
    # resultImage4d, resultMask4d, resultRegisteror4d, resultClahe4d, Ti = register_groupwise(image4D, image4D, clahe4D, dp_rbc, dp_mem, os.path.join(save_dir, 'groupwise/3/'), time_threshold=1, cyclic = cyclic)
    ...
    #     # 'resultDpRbc4d':   final['resultDpRbc4d'],   # ← returning DP‐RBC
    #     # 'resultDpMem4d':   final['resultDpMem4d']    # ← returning DP‐Mem
...
def update_images(image, index_range, average_image):
    ...
    # avg64 = sitk.Cast(average_image, sitk.sitkFloat64)
...
#         registration_four_results = orchestrate_registration_workflow(Image4d, Clahe4d, Rbc4d, Mem4d, savedir, target_weights=[7], index_sets=[{0, 1, 2, 3}], cyclic = False)
...
#         average_rbc = average_4d_image_stack(registration_four_results["resultRbc4d"])
#         average_mem = average_4d_image_stack(registration_four_results["resultMem4d"])
...
#         UpdatedRbc4d = update_images(rbc_to_work, index_range = (4, 7), average_image = average_rbc)
#         UpdatedMem4d = update_images(mem_to_work, index_range = (4, 7), average_image = average_mem)

#         sitk.WriteImage(UpdatedClahe4d, os.path.join(savedir, str(start_index), "ns_clahe_update_01.nii"))
#         registration_four_three_results = orchestrate_registration_workflow(UpdatedImage4d, UpdatedClahe4d, UpdatedRbc4d, UpdatedMem4d, savedir, target_weights=[7], index_sets=[{0, 1, 2, 3}], cyclic = False)
...
#         transfered_rbc_output = apply_transformations_in_sequence(registration_four_results["resultRbc4d"], registration_four_three_results["transformations"], savedir, TransformationPosition.BEGINNING)
#         transfered_mem_output = apply_transformations_in_sequence(registration_four_results["resultMem4d"], registration_four_three_results["transformations"], savedir, TransformationPosition.BEGINNING)
...
#         finalCombinedRbc4d = combine_transformed_and_registered(transfered_rbc_output, registration_four_three_results["resultRbc4d"], index_transfer_range=(0, 4), index_register_range=(1, 4))
#         finalCombinedMem4d = combine_transformed_and_registered(transfered_mem_output, registration_four_three_results["resultMem4d"], index_transfer_range=(0, 4), index_register_range=(1, 4))
...
#         FinalUpdatedRbc4d = process_images(finalCombinedRbc4d, index_range=(0, 7))
#         FinalUpdatedMem4d = process_images(finalCombinedMem4d, index_range=(0, 7))

#         # sitk.WriteImage(finalCombinedClahe4d, os.path.join(savedir, str(start_index), "ns_clahe_update_02.nii"))
        
#         registration_seven_results = orchestrate_registration_workflow(FinalUpdatedImage4d, FinalUpdatedClahe4d, FinalUpdatedRbc4d, FinalUpdatedMem4d, savedir, target_weights=[14], index_sets=[{0, 1, 2, 3, 4, 5, 6}], cyclic = False)
...
def main():
    ...
    # image3D = extract_image_3d(image4D) 
    # clahe3D = extract_image_3d(clahe4D)
    # dp_rbc3D  = extract_image_3d(reg_results['resultDpRbc4d'])
    # dp_mem3D  = extract_image_3d(reg_results['resultDpMem4d'])
    
    # # Final registration 
    # print('ANTs in progress...')   
    # image3D, clahe3D, dp_rbc3D, dp_mem3D = \
    # register_EI_ants(image3D, clahe3D, dp_rbc3D, dp_mem3D, refno, save_dir)
    ...
    # print('ANTs in progress...')   
    # image3D, clahe3D, dp_rbc3D, dp_mem3D = \
    # register_EI_ants(image3D, image3D, dp_rbc3D, dp_mem3D, refno, save_dir)
