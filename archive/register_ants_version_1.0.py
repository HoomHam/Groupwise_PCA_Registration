...
def main():
    ...
    print('ANTs in progress...')   
    image3D, clahe3D = register_EI_ants(image3D, clahe3D, 2, save_dir)
    e6 = time.time()

    image4D = join_image3d_ants(image3D)
    sitk.WriteImage(image4D, os.path.join(save_dir, "reg2s0_clahe.nii"))

    print('ANTs in progress...')   
    image3D, clahe3D = register_EI_ants(image3D, image3D, 2, save_dir)
    # e6 = time.time()
