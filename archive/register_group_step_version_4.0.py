...
# save_dir = '/Volumes/Macintosh HD 2/Work/Analysis/2023-11-23_039CP/reg/'
...
# creare clahe from image
# def create_clahe_from_4d_image(image4d, kernel_size=(16, 16), clip_limit=0.99, nbins=1024):
#     # Get the size of the 4D image
...
#         # Convert the 3D SimpleITK image to a NumPy array
#         array_3d = sitk.GetArrayFromImage(sub_image)

#         # Normalize the image to be between -1 and 1
#         array_3d = 2 * (array_3d - np.min(array_3d)) / (np.max(array_3d) - np.min(array_3d)) - 1

#         # Apply CLAHE to each slice
#         enhanced_array_3d = np.zeros_like(array_3d)
#         for z in range(array_3d.shape[0]):
#             slice_2d = array_3d[z, :, :]
#             enhanced_slice = exposure.equalize_adapthist(slice_2d, kernel_size=kernel_size, clip_limit=clip_limit, nbins=nbins)
#             # enhanced_slice = exposure.equalize_adapthist(slice_2d, ntiles_x=ntiles_x, ntiles_y=ntiles_y, clip_limit=clip_limit, nbins=nbins)
#             enhanced_array_3d[z, :, :] = enhanced_slice

#         # Convert the enhanced NumPy array back to a SimpleITK image
#         enhanced_sub_image = sitk.GetImageFromArray(enhanced_array_3d)
#         enhanced_sub_image.CopyInformation(sub_image)
#         enhanced_images.append(enhanced_sub_image)

#     # Combine the enhanced 3D images into a 4D image
...
def create_clahe_from_4d_image(image4d, kernel_size=(16, 16), clip_limit=0.99, nbins=1024):
    ...
    for i in range(num_images):
        ...
        # Normalize the image to be between -1 and 1
        # array_3d = 2 * (array_3d - np.min(array_3d)) / (np.max(array_3d) - np.min(array_3d)) - 1
        array_3d = array_3d / (np.max(array_3d)) - .0000001
...
def register_groupwise(image, clahe, savedir, enhancednotclahe):
    ...
    if index_sets is not None and target_weights is not None:
        ...
        print('bypassed Enhanced and Internal CLAHE; use iput CLAHE')
        # clahe = create_clahe_from_4d_image(image)
        sitk.WriteImage(clahe, os.path.join(savedir, "clahe_init.nii"))
...
def main():
    ...
    # s0  = time.time()
    # image4D, mask4D, clahe4D = register_groupwise(image, clahe, os.path.join(save_dir, 'groupwise/0/'), True)
    # e0 = time.time()

    # s1  = time.time()
    # # Perform initial groupwise registration (False: USE INTERNAL CLAHE IF ENHANCED)
    # # image4D, mask4D, clahe4D = register_groupwise(image, clahe, os.path.join(save_dir, 'groupwise/1/'), False)
    image4D = sitk.ReadImage('/Volumes/Macintosh HD 2/Work/Analysis/2023-04-14_000KR/reg/groupwise/0/image_groupwise.nii')
    mask4D = sitk.ReadImage('/Volumes/Macintosh HD 2/Work/Analysis/2023-04-14_000KR/reg/groupwise/0/mask_groupwise.nii')
    clahe4D = sitk.ReadImage('/Volumes/Macintosh HD 2/Work/Analysis/2023-04-14_000KR/reg/groupwise/0/clahe_groupwise.nii') 
    # e1 = time.time()
    ...
    image4D, mask4D, clahe4D = register_groupwise(image4D, image4D, os.path.join(save_dir, 'groupwise/2/'), True)
    ...
    # # image4D, mask4D, clahe4D = register_groupwise(image4D, image4D, os.path.join(save_dir, 'groupwise/3/'), True)
    # # # # image4D = sitk.ReadImage('/Volumes/Macintosh HD 2/Work/Analysis/2023-05-15_035EK/reg/groupwise/3/image_groupwise.nii')
    # # # # mask4D = sitk.ReadImage('/Volumes/Macintosh HD 2/Work/Analysis/2023-05-15_035EK/reg/groupwise/3/mask_groupwise.nii')
    # # # # clahe4D = sitk.ReadImage('/Volumes/Macintosh HD 2/Work/Analysis/2023-05-15_035EK/reg/groupwise/3/clahe_groupwise.nii') 
    # # e5 = time.time()
